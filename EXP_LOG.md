...
| 20260909_diffbir_test | 2026-09-08 | submission | λ=0.5 首次 test-100 官方校准提交 | 100 张 test；512 tile/64 overlap/50 steps/fp16/upscale=1；后处理 λ=0.5；GT 不可见 | 官方分数 3.2566，榜单排名第 96；当前前十门槛约 3.8，差距约 0.5434 | 输出与提交闭环完成，但未达到晋级目标；本地 val 的 equal=0.5550 不能直接换算官方分数，且排序信号迁移不足；暂停新提交，先做失配与提速分析 | 远端 manifest 待同步 |
...
- 当前阶段：阶段 7 的首次官方提交已完成，进入“结果解释、失配分析与运行成本评估”暂停期；暂不启动新的 test 或提交。
...
- 决策：λ=0.5 虽然在 5 张 val 上优于 λ=0.7，但首次官方 test 得分仅 3.2566、排名第 96，暂不允许把它视为有效 competition candidate；也不据此继续消耗提交额度。
- 官方校准结论：本地 val 的 equal=0.5550 与官方 3.2566 不同量纲，不能做数值换算；更重要的是，本地指标改善没有迁移为足够的官方排名，说明 val 代表性、指标权重或增强强度仍存在明显失配。
- 下一步计划：暂停新测试；先保留本次失败但完整的 run/提交记录，分析 512 tile、50 steps、单图 DiffBIR 子进程开销与可接受提速方案，再决定是否做小规模验证。
- 闸门：test-100 输出必须 100 张齐全、命名正确、分辨率一致且无失败图片；官方提交前仍需经过 `src/submit/validate.py` 和打包结构检查。
- 保密状态：test 的 GT 仍未公开；已消耗第 1 次官方提交额度，官方只返回分数与排名，没有泄露逐图 GT。
